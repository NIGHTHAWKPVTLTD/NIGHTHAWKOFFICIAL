...
import { 
    getFirestore, doc, setDoc, getDoc, collection, onSnapshot, query, where, orderBy, 
...
let isAdmin = false;
let isStaff = false;
let unsubUser, unsubMatches, unsubTxs, unsubSettings = null;
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        // Initialize Global Settings Listener (so all users get the latest QR / UPI details instantly)
        unsubSettings = onSnapshot(doc(db, 'settings', 'platform'), (snap) => {
            if(snap.exists()) {
                const data = snap.data();
                // Update User UI
                const qrImg = document.getElementById('deposit-qr-display');
                if (data.depositQRUrl) {
                    qrImg.src = data.depositQRUrl;
        ...
        }, (err) => console.warn('Settings Read Access:', err.message));
...
window.savePlatformSettings = async () => {
    if (!isAdmin) return;
    const upi = document.getElementById('admin-set-upi').value;
    const qrUrl = document.getElementById('admin-set-qr').value;
    try {
...
const loadUserData = () => {
    ...
    unsubTxs = onSnapshot(query(collection(db, 'transactions'), where('uid', '==', currentUser.uid), orderBy('timestamp', 'desc')), (snap) => {
...
const renderUserMatches = () => {
    ...
    filtered.forEach(m => {
        ...
        const dt = new Date(m.time).toLocaleString([], {month:'short', day:'numeric', hour:'2-digit', minute:'2-digit'});
...
const loadManagementData = () => {
    ...
    if (isAdmin) {
        ...
        }, (err) => console.warn('Finance Tx Access:', err.message));
    }
};
        ...
        if (m.participantDetails && m.participantDetails.length > 0) {
            rosterHtml = m.participantDetails.map((p, i) => `
                <div class="py-1.5 px-2 border-b border-white/5 last:border-0 flex justify-between items-center hover:bg-white/5 rounded transition">
                    ...
                    <span class="text-[9px] text-gray-500 bg-black px-1.5 py-0.5 rounded border border-white/10">UID: ${p.uid.substring(0,5)}</span>
                </div>`).join('');
            ...
            rosterHtml = m.participants.map((uid, i) => `
                <div class="py-1.5 px-2 border-b border-white/5 last:border-0 text-gray-400">
                    ${i+1}. UID: ${uid.substring(0,8)}... (Legacy)
                </div>`).join('');
        } else {
            ...
            <div class="glass-panel border border-white/10 rounded-2xl p-5 shadow-lg mb-4 hover:border-white/20 transition group relative overflow-hidden">
                ...
                <div class="flex justify-between items-start mb-3">
                    <div>
                        ...
                        <p class="text-[10px] text-gray-400 font-mono mt-1"><i class="fa-solid fa-calendar-day text-blue-400 mr-1"></i>${new Date(m.time).toLocaleString()}</p>
                    </div>
...
document.getElementById('manage-match-form').addEventListener('submit', async (e) => {
    ...
    try {
        if(id) {
            ...
            showToast('New mission deployed.', 'success');
        }
